...
_birthdaySongKeys = []
for k, v in _songs.items():
    if "birthday" in v and v["birthday"]:
        _birthdaySongKeys.append(k)
random.shuffle(_birthdaySongKeys)
_birthdaySongKeys = itertools.cycle(_birthdaySongKeys)
...
def keys():
    ...
    keys = [k for k, v in _songs.items() if "birthday" not in v and (is_christmas or "christmas" not in v)]
    return keys


def birthday_song_keys():
    return _birthdaySongKeys


def get_song(song_key):
    song = _songs[song_key]
